...
def fileCov(na):
	...
	with open('covMatrix.csv', 'r') as f1:
		...
		i = 0
		for row in reader:
			y[i] = np.array(row, double)
			i = i +1
			
	with open('rMean.csv', 'r') as f2:
		reader = csv.reader(f2)
		for row in reader:
			rmean = np.array(row, double)
	return rmean, y
...
# The 10x10 matrix
A_t = A.T
A_t = A_t.astype(numpy.double) #float32
c_double_p = ctypes.POINTER(ctypes.c_double)
A1_p = A_t.ctypes.data_as(c_double_p)
# The 10x1 matrix
x1= np.empty([na+2])
x1= x1.astype(numpy.double)
X_p = x1.ctypes.data_as(c_double_p)
# The 10x1 matrix with covariance
b_t=b.T
b_t = b_t.astype(numpy.double)
b1_p = b_t.ctypes.data_as(c_double_p)

x = solve(A, b)

libculaC.culaDgesv(10,1,A1_p,10,X_p,b1_p,10)
a = np.fromiter(b1_p, dtype=np.double, count=10)
print a
